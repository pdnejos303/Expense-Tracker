...
- **Typography**: MUI default (Roboto). Let data breathe but don't waste space — favor compact layouts that show more information per screen.
...
- **Theme**: Light mode primary. Dark mode not currently implemented.
...
3. **Compact density** — Favor information density over whitespace. Show more rows, more charts, more context per viewport. Mobile should still feel rich, not stripped down.
4. **Consistent visual language** — Income is always green/positive, expenses are always red/negative. Budget progress uses the same color scale everywhere. Patterns should be learnable.
5. **Accessible by default** — Meet WCAG AA standards: sufficient color contrast, keyboard navigable, screen-reader friendly labels. Test color choices against color blindness.

### Tech Constraints
- React 18 + Vite, MUI v6 component library
- Recharts for data visualization
- Firebase backend (Firestore + Auth)
- Thai language (th locale), THB (฿) currency formatting
